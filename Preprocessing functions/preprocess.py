...
def text_to_numeric_one_category(train_data, test_data, label, category, action_list):
    ...
    if len(train_data[category].value_counts().index.values) != len(test_data[category].value_counts().index.values):
        ...
        while True:
            ...
            startvalue = input("What value would you like the new feature by default?")
            train_cpy[new_name] = startvalue
            print("All the data is now initialized to the value of:", startvalue)
            print("You will now be asked to give the next value (those that are not",startvalue,"), while all the other data will stay with the value of:", startvalue)
            del cols[:]
            del vals[:]
            for i in range(0,num_cats-1):
                ...
                while True:
                    newvl = input("Type the new value:")
                    try:
